...
public class ClusterManagementServiceFactory {

  public static ClusterManagementService create(ClusterManagementContext ctx) {
    switch (ctx) {
      case FOR_LOCATOR:
  ...
  public static ClusterManagementService create() {
    if (InternalLocator.getLocator() != null) {
      return create(ClusterManagementContext.FOR_LOCATOR);
    }

    try {
      Cache cache = CacheFactory.getAnyInstance();
      if (cache != null && cache.isServer()) {
        return create(ClusterManagementContext.FOR_SERVER);
      }

      ClientCache clientCache = ClientCacheFactory.getAnyInstance();
      if (clientCache != null) {
        return create(ClusterManagementContext.FOR_CLIENT);
      }
